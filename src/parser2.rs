...
pub trait Builder where Self::Key: fmt::Display, Self::Value: ShowType {
    type Table;
    type Array;
    type Key;
    type Value;
    ...
    fn insert_array(t: &mut Self::Table, keys: &[Self::Key], value: Self::Value);
    fn insert_table(t: &mut Self::Table, keys: &[Self::Key], value: Self::Table);
}
...
impl Builder for SimpleBuilder {
    ...
    fn insert_array(t: &mut TomlTable, keys: &[String], value: Value) { unimplemented!() }
    fn insert_table(t: &mut TomlTable, keys: &[String], value: TomlTable) { unimplemented!() }
}
...
impl<'a, B:Builder> Parser<'a, B> {
    ...
    /// }
    /// ```
    pub fn new(s: &'a str, b: B) -> Parser<'a, B> {
        Parser {
            input: s,
            cur: s.char_indices(),
            errors: Vec::new(),
    ...
    fn finish_string(&mut self,
        ...
        fn escape<B:Builder>(me: &mut Parser<B>, pos: usize, multiline: bool) -> Option<char> {
    ...
    fn array(&mut self, _start: usize) -> Option<B::Array> {
        if !self.expect('[') { return None }
        let mut ret = Vec::new();
        fn consume<B:Builder>(me: &mut Parser<B>) {
            loop {
                me.ws();
